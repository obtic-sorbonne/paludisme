...
class TimeGroup:
    def __init__(self, date_str: str):
        self.date = date_str
        self.pages: list[tuple[DocumentSection, Page]] = []
...
# ─────────────────────────────────────────────────────────────────────────────
# FIX 6 — Extract lab value from dated markdown table column
# ─────────────────────────────────────────────────────────────────────────────
def extract_lab_table_by_date(text: str, param: str, target_date: str) -> str:
    """
    Extract a lab value from a markdown bilan table by matching the date column.

    Handles tables like:
      |                 | 21.9.06 | 24.9.06 | 25.9.06 |
      | ASAT (UI/l)     | 102     | 131     | 169     |

    Args:
        text:        Full anonymized text to search
        param:       Parameter label to find e.g. "ASAT", "Hémoglobine"
        target_date: Date in any format e.g. "24/09/2006"

    Returns:
        Numeric value as string, or "" if not found
    """
    def date_variants(d: str) -> list:
        variants = [d]
        m = re.match(r"(\d{1,2})[/.](\d{1,2})[/.](\d{2,4})", d)
        if m:
            day, month, year = m.group(1), m.group(2), m.group(3)
            year2 = year[-2:]
            year4 = ("20" + year2) if len(year) == 2 else year
            variants += [
                f"{day}.{int(month)}.{year2}",
                f"{day}.{int(month)}.{year4}",
                f"{day}/{month}/{year2}",
                f"{day}/{month}/{year4}",
                f"{int(day)}.{int(month)}.{year2}",
                f"{int(day)}/{int(month)}/{year4}",
            ]
        return list(set(variants))

    date_vars = date_variants(target_date)
    ...
    for table_match in table_pattern.finditer(text):
        ...
        # Find which column index matches our target date
        col_idx = None
        for dv in date_vars:
            for i, cell in enumerate(header_cells):
                if dv.lower() in cell.lower():
                    col_idx = i
                    break
        ...
        if col_idx is None:
            continue  # this table doesn't have our date

        # Find the row matching our parameter
        param_norm = norm(param)
        for data_line in lines[2:]:
            cells = [c.strip() for c in data_line.strip("|").split("|")]
            if not cells:
                continue
            if param_norm in norm(cells[0]):
                if col_idx < len(cells):
                    val = cells[col_idx].strip().rstrip("+- ")
...
def fill_timepoint(
    ...
    # ── FIX 6: Lab values from dated markdown tables for J3 and J30 ──────────
    if prefix in ("J3", "J30") and tp_date:
        full_text_for_table = all_clinical_text + "\n" + grp.text()

        table_lab_params = {
            f"ASAT_{prefix}":       "ASAT",
            f"ALAT_{prefix}":       "ALAT",
            f"bilirubine_{prefix}": "Bilirubine totale",
            f"Urée_{prefix}":       "Urée",
            f"Créatinine_{prefix}": "Créatinine",
            f"CRP_{prefix}":        "CRP",
        }
        table_hb_params = {
            f"hémoglobine_{prefix}":     "Hémoglobine",
            f"plaquettes_{prefix}":      "Plaquettes",
            f"globules_blancs_{prefix}": "Leucocytes",
        }

        for col, param_name in table_lab_params.items():
            if col in COLUMNS and not row[col]:
                val = extract_lab_table_by_date(
                    full_text_for_table, param_name, tp_date
                )
                if val:
                    row[col] = val

        for col, param_name in table_hb_params.items():
            if col in COLUMNS and not row[col]:
                val = extract_lab_table_by_date(
                    full_text_for_table, param_name, tp_date
                )
...
def extract_row(
    ...
    # ── FIX 5: Moustiquaire from CNR field ───────────────────────────────────
    v = get_cnr(all_cnr, "Protection Personnelle Anti-Moustiques")
    if v:
        nv = norm(v)
        if "non" in nv:
            row["moustiquaire_imp"] = "Non"
        elif "oui" in nv:
            row["moustiquaire_imp"] = "Oui"
        elif "nsp" in nv:
            row["moustiquaire_imp"] = "NSP"
        else:
            row["moustiquaire_imp"] = v

    # ── FIX 4: Goutte épaisse J0 from CNR (includes "Non fait") ──────────────
    v = get_cnr(all_cnr, "Goutte épaisse")
    if not v:
        # get_cnr skips "nsp" but "Non fait" is not in the skip list — however
        # we need to bypass the nsp/none filter for this field specifically
        for sec in all_cnr:
    ...
    # Symptom onset
    v = get_cnr(all_cnr, "Date des Premiers Symptômes de cet accès") or \
...
def process_patient(anon_file: Path, config: dict) -> Optional[dict]:
    ...
    show = ["Sexe", "Age", "Lieu_séjour", "chimioprophylaxie", "moustiquaire_imp",
            "Goutte_épaisse_J0", "Type_paludisme", "gravité_palu",
            "Traitement_antipalu", "Hospitalisation", "Durée_hospit",
            ...
            "Date_PEC_J3", "fièvre_J3", "Température_J3", "Frottis_sanguin_J3",
            "ASAT_J3", "ALAT_J3",
