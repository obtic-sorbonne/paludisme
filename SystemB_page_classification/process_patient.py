...
def get_patient_id(folder_path: Path) -> str:
    """Return a unique patient ID from the folder name, keeping year prefix.
    e.g. '2006 RDB 0192' → 'RDB_0192'  (for display and output folder)
    """
...
def get_cache_key(folder_path: Path) -> str:
    """Return a globally unique cache key for GLM-OCR.
    Uses the FULL absolute path sanitized to avoid any collisions:
      /data/2006/RDB_0192  →  2006_RDB_0192
      /data/2007/RDB_0192  →  2007_RDB_0192
    Even if folder names are identical across year folders, this is always unique.
    """
    import re
    # Use last 2 path components: year_folder + patient_folder
    # e.g. /path/to/2006 RDB 0015  → "2006_RDB_0015"
    parts = list(folder_path.parts)
    # Take the patient folder name
    patient_part = folder_path.name.strip()
    # Check if parent folder looks like a year (4 digits)
    parent_part = folder_path.parent.name.strip()
    if re.match(r"^\d{4}$", parent_part):
        key = f"{parent_part}_{patient_part}"
    else:
        key = patient_part
...
# ── Step 1: GLM-OCR ───────────────────────────────────────────────────────────
def run_glm_ocr(pdf_path: Path, patient_id: str = "") -> dict:
    doc_id    = pdf_path.stem
    # Build a globally unique cache key using patient_id + doc_id.
    # This prevents cache collisions when:
    #   - Two patient folders share the same name (e.g. 2006/RDB_0192 vs 2007/RDB_0192)
    #   - Two different patients have files with the same filename (e.g. DOC_00185.pdf)
    # Cache key format: PATIENT_ID__DOC_ID  (double underscore separator)
    cache_key = f"{patient_id}__{doc_id}" if patient_id else doc_id
    glm_dir   = Path(GLM_OCR_BASE) / cache_key
    ...
    ok, out = run_cmd(f'bash "{GLM_SCRIPT}" "{pdf_path}" "{glm_dir}"', timeout=1800)
    if not ok:
...
def process_patient(folder: str, debug: bool = False) -> str:
    ...
    # Globally unique cache key — includes year folder to avoid collisions
    # e.g. 2006/RDB_0192 → "2006_RDB_0192"  vs  2007/RDB_0192 → "2007_RDB_0192"
    patient_cache_key = get_cache_key(folder)
    ...
    for i, pdf_path in enumerate(pdfs, 1):
        ...
        glm_info = run_glm_ocr(pdf_path, patient_id=patient_cache_key)
    ...
    failed_docs = [d["doc_id"] for d in doc_outputs if d["type"] == "failed"]
    ...
    if failed_docs:
        print(f"  ⚠️  WARNING: {len(failed_docs)} document(s) failed — data NOT in output:")
        for d in failed_docs:
            print(f"     ❌ {d} — MISSING from patient file")
        print(f"  To fix, delete bad cache and rerun:")
        print(f"     rm -rf ~/glm_ocr_runs/*__{failed_docs[0]}")
        print(f"     bash run_pipeline.sh \"{folder}\"")
        print()
